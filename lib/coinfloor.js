...
var MIN_RECONNECT_INTERVAL = 100; // 0.1 seconds
var MAX_RECONNECT_INTERVAL = 60000; // 1 minute
...
var _pending_orders = [];
var _reconnect_delay = MIN_RECONNECT_INTERVAL;
var _reconnect_timer_id;

Coinfloor.on("open", function () {
	console.log("Coinfloor: WebSocket connected to " + (_url || Coinfloor.DEFAULT_URL));
});

Coinfloor.on("error", function (event) {
	console.error("Coinfloor: WebSocket error:", event);
	Coinfloor.trigger("close", event);
});

Coinfloor.on("close", function () {
	console.log("Coinfloor: WebSocket disconnected");
	if (_reconnect_timer_id) {
		clearTimeout(_reconnect_timer_id);
	}
	_reconnect_timer_id = setTimeout(function () {
		Coinfloor.connect(_url, function () {
			_reconnect_delay = MIN_RECONNECT_INTERVAL;
		});
	}, _reconnect_delay);
	_reconnect_delay = Math.min(_reconnect_delay * 1.5 | 0, MAX_RECONNECT_INTERVAL);
});

Coinfloor.on("Welcome", function () {
	Coinfloor.authenticate(_config.user_id, _config.cookie, _config.passphrase, function (msg) {
		if (msg.error_code) {
			console.error("Coinfloor: Failed to authenticate:", msg);
		}
		else {
			console.log("Coinfloor: Authentication succeeded");
			var orders = _pending_orders;
			_pending_orders = [];
			for (var i = 0; i < orders.length; ++i) {
				var order = orders[i];
				trade(order.satoshis, order.opts, order.callback);
			}
		}
	});
});
...
function trade(satoshis, opts, callback) {
	if (!Coinfloor.isConnected()) {
		callback(new Error("Coinfloor is disconnected"));
		return;
	}
	var tonce = opts && opts.tonce || ((opts || (opts = {})).tonce = Date.now());
	_pending_orders.push({
		satoshis: satoshis,
		opts: opts,
		callback: callback,
	});
	...
	Coinfloor.request({
		...
		_pending_orders.shift();
		callback(msg.error_code && msg.error_code != 3 ? new Error(msg.error_msg) : null);
	});
}
